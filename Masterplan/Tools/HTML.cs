...
namespace Masterplan.Tools
{
    ...
    // This class holds the global setting for the table width.
    /// <summary>
    /// Generates the layout text for the HTML tables
    /// and sets Maximum Width property and font sizes
    /// </summary>
    /// 
    public class CssGenerator
    {
        // Mock data for demonstration (actual data structures may vary)
        // private string[] pt_sizes = new string[] { "8", "9", "10", "11", "12", "14", "16", "18", "10" };
        // the application will pass the font size as a parameter

        // --- SINGLETON IMPLEMENTATION START ---
        // 1. Private static field to hold the single instance to prevent overwrites to the default
        private static readonly CssGenerator _instance = new CssGenerator();

        // 2. Public static property to provide the global access point

        /// <summary>
        /// CssGenerator Instance to maintain edit format layout 
        /// </summary>
        public static CssGenerator Instance
        {
        ...
        // 4. Backing field for the LayoutStyle property
        private string _layoutStyle = "wide"; // Default to wide layout for the table

        /// <summary>
        /// 5. Calculated Property: Automatically returns the correct max-width CSS value.
        /// </summary>
        public string MaxWidth
        {
            get
            {
                if (_layoutStyle.Equals("narrow", StringComparison.OrdinalIgnoreCase))
                {
                    return "400px";
                }
                return "99%";
        ...
        /// <summary>
        /// 6. Property with Custom Logic: This is the variable the user changes.
        /// </summary>
        public string LayoutStyle
        {
            ...
            set
            {
                if (!value.Equals(_layoutStyle, StringComparison.OrdinalIgnoreCase))
                {
                    _layoutStyle = value;
                }
            }
        ...
        /// <summary>
        /// Generates the table CSS rule (long layout).
        /// </summary>
        /// <returns>The complete CSS rule string for the table element.</returns>
        public string GenerateTableStyle(string fontSize) // <-- Font size parameter added
        {
            string calculatedMaxWidth = this.MaxWidth;

            string longLayout = string.Format(
          "table {{ font-size: {0}pt; border-color: #BBBBBB; border-style: solid; border-width: 1px; border-collapse: collapse; table-layout: fixed; min-width: 300px; max-width: {1} }}",
                fontSize, // <-- Using the new parameter here
                calculatedMaxWidth  // {1} - Max width value (changes based on LayoutStyle)
            );
        ...
        /// <summary>
        /// Generates the short HTML table style (short layout).
        /// </summary>
        /// <returns>The HTML string with the inline style.</returns>
        public string GenerateShortStyle()
        {
            string calculatedMaxWidth = this.MaxWidth;
            string shortLayout;

            if (_layoutStyle == "wide")
            {
                shortLayout = string.Format("<TABLE style = \"min-width: 300px; max-width: {0}\">",
                  calculatedMaxWidth  // This holds "400px" or "99%", reflecting the current persistent LayoutStyle
                        );
            }
            else
            {
                shortLayout = string.Format("<TABLE style = \"min-width: 350px; max-width: {0}\">",
                  calculatedMaxWidth  // This holds "400px" or "99%", reflecting the current persistent LayoutStyle
                        );
            }
    ...
    class HTML
    {
        ...
        public static List<string> GetStyle(DisplaySize size)
        {
            ...
            // Generate the dynamic table layout
            // (400px or 99%)
            string font_size = pt_sizes[8].ToString();
            var generator = CssGenerator.Instance;
            string layoutField = generator.GenerateTableStyle(font_size);
            ...
            // LOAD external css formatting if available
            bool loaded = false;
            ...
            // USE internal css formatting if no external css loaded
            if (!loaded)
            {
                ...
                lines.Add(layoutField);  //dynamic table layout width and font size
